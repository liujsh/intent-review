...
- [x] 6. 实现最小 `intent-review:plan` Skill
...
- [x] 8. 冻结 Task Store Schema 与存储策略
...
- [x] 10. 实现 `intent-review:init` 与 `intent-review:resume`
...
- [x] 14. 实现 `intent-review:impl` Skill
...
- [ ] 15. 实现审查预算与分层路由
  - 已完成输入/文件/轮数/任务 Token 硬预算与超限失败语义；文件哈希缓存和低成本 Reviewer 自动升级路由留待后续版本。
